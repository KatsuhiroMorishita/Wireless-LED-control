/* program name: type03_lebel_meter                     */
/* author:  Katsuhiro MORISHITA                         */
/* purpose: 音量ゲージのようなレベルメータ制御を行う。         */
/* create:  2015-05-05                                  */
/* license:  MIT                                        */
/* format 1: header_v2, turn_on_amount, color, NA, ID   */
...
// LED control
// 全てのLEDを単色に光らせる
void light(int turn_on_amount, int color_lebel, int NA)
{
  // 変数制限
  if(turn_on_amount > NUMPIXELS) turn_on_amount = NUMPIXELS;
  if(color_lebel > 100) color_lebel = 100;
  
  int r, g, b;
  colmap.GetColor((double)color_lebel / 100, &r, &g, &b);
  
  for(int i = 0; i < NUMPIXELS; i++)
  {
    if(i + 1 <= turn_on_amount)
